...
$('.select').html(
    data.map(
        n => `<div class="select__item">${n.name}</div>`
    ).join('')
)

var select = $('.select').draggabilly({
    axis: 'y'
})


select.on('dragMove', function(_1, _2, move) {
    var t = parseFloat($(this).css('top').split('px')[0]) + move.y
    var idx = 0

    if(t >= 25 * 4) {
        idx = 0
    }
    else if(t <= ($('.select__item').length - 3) * -25) {
        idx = $('.select__item').length - 1
    } else {
        idx = Math.round(Math.abs(t - 3 * 25) / 25)
    }

    $(this).data('idx', idx)

    $('.select__item').removeClass('js-curr js-prev-1 js-prev-2 js-prev-3 js-next-1 js-next-2 js-next-3 js-prev-4 js-prev-5 js-next-4 js-next-5')
    var curr = $('.select__item').eq(idx).addClass('js-curr')
    curr.prev().addClass('js-prev-1')
        .prev().addClass('js-prev-2')
        .prev().addClass('js-prev-3')
        .prev().addClass('js-prev-4')
        .prev().addClass('js-prev-5')
    curr.next().addClass('js-next-1')
        .next().addClass('js-next-2')
        .next().addClass('js-next-3')
        .prev().addClass('js-prev-4')
        .prev().addClass('js-prev-5')
})

select.on('dragEnd', function() {
    var idx = $(this).data('idx')
    if(idx <= 0) {
        $(this).animate({ top: 25 * 3 + 'px'})
    } else if(idx >= $('.select__item').length - 1) {
        $(this).animate({ top: ($('.select__item').length - 4) * -25 + 'px'})
    }
})

$('.js-select-checked').on('click', _ => {
